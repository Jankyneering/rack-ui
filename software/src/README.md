...
| `0x0E` | R/W | `0x03` | Active animation, see [below](#animation-register-0x0e). |
...
| `0x03` | `PULSING` | All LEDs smoothly ramp up and down in brightness. **Default at power-on.** |
...
Holds the timing setting of the animation selected through register `0x0E`.
Writing a new animation id to `0x0E` loads that animation's default into `0x0F`;
a subsequent write to `0x0F` adjusts the timing while the animation keeps
running. A value of `0` is ignored and keeps the current setting.

| Animation | Scale | Default | Meaning |
| --- | --- | --- | --- |
| `IDLE` | - | `0x00` | No timing setting. |
...
| `FOLLOWING` / `POINT` / `GAUGE` | - | `0x00` | No runtime timing setting. |
...
# Switch to the pulsing animation (power-on default); its default 10 ms pulse
# delay is loaded into register 0x0F
