...
/* Private variables */
uint8_t device_memory[REG_COUNT];
...
int main(void) {
    ...
    for(int i=0; i<REG_COUNT; i++) {
        if (i < REG_RW_LIMIT) device_memory[i] = i+12; // Start RW regs at 0
        else if (i < REG_RO_LIMIT) device_memory[i] = 0xAA; // Start RO regs at 0xAA
        else device_memory[i] = DEFAULT_VAL; // Everything else is 0x42
    }
